    ...
    pub async fn run(self) -> Result<()> {
        ...
        let artifact = {
            ...
            if is_storage_layout_empty(&artifact.storage_layout) && auto_detect {
                ...
                sh_eprintln!("The requested contract was compiled with {version} while the minimum version for storage layouts is {MIN_SOLC} and as a result the output may be empty.");
...
async fn fetch_and_print_storage<P: Provider<T, AnyNetwork>, T: Transport + Clone>(
    ...
    if is_storage_layout_empty(&artifact.storage_layout) {
        sh_eprintln!("Storage layout is empty.");
        Ok(())
    } else {
